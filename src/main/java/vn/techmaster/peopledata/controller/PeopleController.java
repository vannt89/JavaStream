...
public class PeopleController {
    ...
    @GetMapping("maleFemaleRatioAllData")
    ...
    public ResponseEntity<Map<String, List<Person>>> findDeveloperOf3City() {
        return ResponseEntity.ok().body(personRepo.findDeveloperOf3City());
    }

    //2. Tìm ra thành phố có độ tuổi trung bình của lập trình viên trẻ nhất
    @GetMapping("youngestdevs")
    public ResponseEntity<List<Map.Entry<String, Double>>> findCityHasYoungestDev() {
        return ResponseEntity.ok().body(personRepo.findCityHasYoungestDev());
    }

    //3. Liệt kê tỷ lệ nam/nữ ở từng thành phố
    @GetMapping("malefemaleratio")
    public ResponseEntity<Map<String, Double>> maleFemaleRatioOnPerCity() {
        return ResponseEntity.ok().body(personRepo.maleFemaleRatioOnPerCity());
    }

//    //4. Tính mức lương trung bình của tất cả những người trên 30 tuổi
    @GetMapping("avgsalarypeopleabove30")
    public ResponseEntity<Double> calAvgSalaryPeopleAbove30() {
        return ResponseEntity.ok().body(personRepo.calAvgSalaryPeopleAbove30());
    }
}
